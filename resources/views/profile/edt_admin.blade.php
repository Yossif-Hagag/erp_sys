...
<x-nav-link :href="route('edt_admin',$admin->id)" :active="request()->routeIs('edt_admin')" style="">
    {{ __('تعديل المسئول') }}
</x-nav-link>
@endsection
...
<x-guest-layout>
    ...
    @if ($errors->count() > 0)
        ...
        @endforeach
    @endif
    <form method="POST" action="{{ route('update_admin',$admin->id) }}" style="direction: rtl;">
        @csrf
        ...
        <div class="mt-4">
            ...
            <x-text-input id="phone" class="block mt-1 w-full" type="text" name="phone" :value="old('phone',$admin->phone)"
        ...
        <div class="mt-4">
            ...
            <x-text-input id="password" class="block mt-1 w-full" type="text" name="password" value="{{null}}" required autocomplete="password" />
