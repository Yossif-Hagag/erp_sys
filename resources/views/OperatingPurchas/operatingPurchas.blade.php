        ...
        <div class="content-body row m-0 overflow-y-scroll">
            ...
            <div class="content col-10 py-1 overflow-hidden">
                ...
                <div class="buttons position-relative row m-0 py-3">
                    <div class="col-6">
                        <div class="wrap w-100">
                            <form action="{{ route('operation_purchas.search') }}" method="get"
                                id="operation_purchas-form" class="search w-100 position-relative d-flex">
                                <button type="submit" class="searchButton text-center text-white">
                                    <i class="fa fa-search"></i>
                                </button>
                                <input type="text" id="operation_purchas_keyword"
                                    class="searchTerm w-50 border-end-0 text-center"
                                    placeholder="ابحث بالعميل أو التكلفة . . ." />
                    ...
                    <div class="col-6 text-start">
                        <a href="{{route('operation_purchas_pdf')}}" class="btn btn-dark">
                            <i class="fa fa-download ms-2"></i>PDF
                        </a>
                        <a href="{{ route('add_operation_purchas') }}" class="btn btn-dark">
                        ...
                        <div class="modal fade" id="total" tabindex="-1" aria-labelledby="totalLabel"
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body fs-3 d-block text-center" id="total-sell-price"> التكلفة:
                                        <span>{{ $totalsellPrice }}</span>
                                    </div>
                        ...
                        <!-- End Total Tag -->
                        <a href="{{ route('purchas') }}" class="btn btn-dark">
                ...
                <div class="table-data pe-1 h-auto">
                    <table class="table table-striped text-center">
                        ...
                        <tbody id="data-body">
                            @foreach ($OperationgPurchas as $op_purchas)
                                <tr>
                                    <td>{{ $op_purchas->code }}</td>
                                    <td>
                                        <a class="btn btn-sm collection text-primary"
                                            href="{{ asset('storage/' . $op_purchas->operation_document) }}"
                                    ...
                                    <td>{{ $op_purchas->start_price }}</td>
                                    <td>{{ $op_purchas->cost }}</td>
                                    <td>
                                        <a type="button" id="edt_operation_purchas"
                                            href="{{ route('edt_operation_purchas', $op_purchas->id) }}"
                                            class="lh-1 p-2 btn btn-sm btn-dark"><i
                                            class="fa-solid fa-edit"></i></a>
                                    </td>
                                    <td>
                                        <form method="POST" action="{{ route('delete_operation_purchas', ['id' => $op_purchas->id]) }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                    ...
                    <div class="paginate px-4" style="direction: ltr;">{{ $OperationgPurchas->links() }}</div>
                </div>
                <!-- End Data -->
            </div>
            <!-- End Content -->
