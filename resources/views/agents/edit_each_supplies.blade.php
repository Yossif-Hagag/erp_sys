...
<head>
  ...
  <title>تعديل ما تم توريده || العربي جروب</title>
...
<body>
  ...
  <div class="home overflow-hidden">
    ...
    <div class="content-body row m-0">
      ...
      <div class="content col-10 flex-column d-flex  py-5 overflow-y-scroll h-100">
        <div id="edtproduct">
          ...
          @endforeach
          @endif
          <form method="POST" action="{{route('update_each_supplies',$each_supplies->id)}}"
            class="w-25 mx-auto rounded-4 p-4 text-center">
            @csrf
            <div class="mb-3">
              <label for="pro_num" class="form-label fs-4 mb-5"><i class="fa-solid fa-edit"></i>  تعديل ما تم توريده</label>
              <select class="form-select select2" id="pro_num" aria-label="Default select example" name="pro_num" required>
                <option value="">اختر</option>
                @foreach ($products as $product)
                    <option value="{{ $product->code }}" @if(old('pro_num', $each_supplies->pro_num) == $product->code) selected @endif>{{ $product->name }}</option>
                @endforeach
            </select>
          </div>
    ...
    <input type="number" name="number_of_product"
        ...
        placeholder="عدد المنتج" aria-label=""  value="{{ old('number_of_product', $each_supplies->number_of_product) }}"
      ...
      <input type="number" name="sell_price"
                        class="form-control mb-3"
                        placeholder="سعر البيع" aria-label="" value="{{ old('sell_price', $each_supplies->sell_price) }}"
                        required>
            </div>
            <div class="mb-4 text-end">
      ...
      <input type="number" name="buy_price"
                        class="form-control mb-3"
                        placeholder="سعر الشراء" aria-label="" value="{{ old('buy_price', $each_supplies->buy_price) }}"
                        required>
            </div>
