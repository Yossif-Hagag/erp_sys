...
<head>
    ...
    <title>تعديل إيجار || العربي جروب</title>
                ...
                <div id="edtRentals">
                    ...
                    <form action="{{ route('update_rental',$rentals->id) }}"
                        ...
                        <div class="mb-4 d-block">
                            ...
                            <select class="form-select select2" id="item-num" aria-label="Default select example" name="pro_num"
                                required>
                                <option selected value="{{ old('pro_num', $rentals->pro_num)}}">
                                    @foreach ($pros as $pro)
                                    @if ($pro->code == $rentals->pro_num)
                                    {{ $pro->name }}
                                    @endif
                                    @endforeach
                                </option>
                        ...
                        <div class="mb-4 d-block">
                            ...
                            <input type="number" class="form-control" id="number_of_product" name="number_of_product"
                                value="{{old('number_of_product',$rentals->number_of_product)}}" required />
                        </div>
                        <div class="mb-4 d-block">
                            <label for="rent_price" class="form-label">سعر الإيجار <span class="text-danger">*</span></label>
                            <input type="number" class="form-control" id="rent_price" name="rent_price"
                                value="{{old('rent_price',$rentals->rent_price)}}" required />
                        </div>
                        <div class="mb-4 d-block">
                            <label for="rent_period" class="form-label">فترة الإيجار باليوم <span class="text-danger">*</span></label>
                            <input type="number" class="form-control" id="rent_period" name="rent_period"
                                value="{{old('rent_period',$rentals->rent_period)}}" required />
                        </div>
                        <div class="mb-4 d-block">
                            <label for="Tenant_name" class="form-label">اسم المستأجر <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="Tenant_name" name="Tenant_name"
                                value="{{old('Tenant_name',$rentals->Tenant_name)}}" required />
                        </div>
                        <div class="mb-4 d-block">
                            <label for="Tenant_phone" class="form-label">تليفون المستأجر <span class="text-danger">*</span></label>
                            <input type="number" class="form-control" id="Tenant_phone" name="Tenant_phone"
                                value="{{old('Tenant_phone',$rentals->Tenant_phone)}}" required />
                        </div>
                        <div class="mb-4 d-block">
                            <label for="Tenant_address" class="form-label">عنوان المستأجر <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="Tenant_address" name="Tenant_address"
                                value="{{old('Tenant_address',$rentals->Tenant_address)}}" required />
                        </div>
