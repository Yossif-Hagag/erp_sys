            ...
            <div class="content col-10 flex-column align-items-center d-flex  justify-content-between p-0 h-auto">
                ...
                <div class="options text-center w-100" id="options">
                    <ul id="options-list">
                        ...
                        <li>
                            <a href="{{ route('audit_rental') }}" id="audit_rental"
                        ...
                        <li>
                            <a href="{{ route('audit_scrap') }}" id="audit_scrap" class="btn btn-sm btn-light ms-2">
                                سكراب
                            </a>
                        </li>
                        <li>
                ...
                <div class="table-data pe-1 overflow-y-scroll w-100">
                        ...
                        <tbody id="data-body">
                            @foreach ($allRen as $x)
                                <tr>
                                    ...
                                    <td>{{ $x->event }}</td>
                                    <td>
                                        @foreach ($rental as $pro)
                                            @if ($x->auditable_id == $pro->id)
                                                {{ $pro->pro_name }}
                                            @endif
                                        @endforeach
                                    </td>
                                    <td>
                                        @foreach ($x->old_values as $key => $value)
                                            <div>
                                                ...
                                                @elseif ($key == 'id' || $key == 'supplier_id' || $key == 'purchas_id' || $key == 'pro_id')
                                                    @continue;
                                                @elseif ($key == 'rent_price')
                                                    <span class="text-danger">سعر التأجير</span>
                                                @elseif ($key == 'store_name')
                                                    <span class="text-danger">إسم المخزن</span>
                                                @elseif ($key == 'rent_period')
                                                    <span class="text-danger">فترة التأجير باليوم</span>
                                                @elseif ($key == 'Tenant_name')
                                                    <span class="text-danger">إسم المستأجر</span>
                                                @elseif ($key == 'Tenant_phone')
                                                    <span class="text-danger">تلفون المستأجر</span>
                                                @elseif ($key == 'Tenant_address')
                                                    <span class="text-danger">عنوان المستأجر</span>
                                                @elseif ($key == 'purchas_sup_price' || $key == 'purchas_sup_price')
                                                    <span class="text-danger">سعر التوريد</span>
                                                @else
                                                    <span class="text-danger">{{ $key }}</span>
                                                @endif
                                                :
                                                @if ($value == 'stock')
                                                    <span class="text-success badge bg-body-secondary">المخزن</span>
                                                @elseif ($value == 'purchas')
                                                    <span class="text-success badge bg-body-secondary">المشتريات</span>
                                                @else
                                                    {{ $value }}
                                                @endif
                                            </div>
                    ...
                    <div class="paginate px-4" style="direction: ltr;">{{ $allRen->links() }}</div>
                </div>
                <!-- End Data -->
