        ...
        <div class="content-body row m-0">
            ...
            <div class="content col-10 mt-5 overflow-y-scroll h-100">
                ...
                <div id="addpo" style="margin-bottom: 5rem">
                    <form action="{{ route('update_po', $po->id) }}" enctype="multipart/form-data" method="POST"
                        ...
                        <div class="mb-4 d-block">
                            ...
                            <input type="text" class="form-control" id="snum" name="snum" required
                                value="{{ old('snum', $po->snum) }}" />
                        </div>
                        <div class="mb-4 d-block">
                            <label for="from" class="form-label">العميل <span class="text-danger">*</span></label>
                            <select class="form-select select2" id="from" aria-label="Default select example"
                                name="from" required>
                                <option selected value="{{ old('from', $po->from) }}">
                        ...
                        <div class="mb-4 d-block">
                            <label for="po_value" class="form-label">القيمة <span
                                    class="text-danger">*</span></label>
                            <input type="number" class="form-control" id="po_value" name="po_value" required
                                value="{{ old('po_value', $po->po_value) }}" />
                        </div>
                        <div class="mb-4 d-block">
                            <label for="collection_date" class="form-label">تاريخ التحصيل <span
                                    class="text-danger">*</span></label>
                            <input type="dateTime-local" class="form-control" id="collection_date"
                                name="collection_date" required
                                value="{{ old('collection_date', $po->collection_date) }}" />
                        </div>
                        <div class="mb-4 d-block">
                            <label for="po_file" class="form-label">مرفق ال po <span
                                    class="text-danger">*</span></label>
                            <input type="file" class="form-control" id="po_file" name="po_file" />
                        </div>
                        <div class="mb-4 d-block">
                            <label for="" class="form-label">عرض مرفق ال po المحفوظ <span
                                    class="text-danger">*</span></label>
                            @if (isset($po->po_file))
                                <a href="{{ asset('storage/' . $po->po_file) }}" target="_blank"
                                    class="form-control text-bg-dark text-decoration-none">{{ substr($po->po_file, 3) }}</a>
                            @endif
                        </div>
                        <div class="mb-4 d-block">
                            <label for="sale_present_file" class="form-label">عرض السعر <span
                                    class="text-danger">*</span></label>
                            <input type="file" class="form-control" id="sale_present_file"
                                name="sale_present_file" />
                        </div>
                        <div class="mb-4 d-block">
                            <label for="" class="form-label">عرض السعر المحفوظ <span
                                    class="text-danger">*</span></label>
                            @if (isset($po->sale_present_file))
                                <a href="{{ asset('storage/' . $po->sale_present_file) }}" target="_blank"
                                    class="form-control text-bg-dark text-decoration-none">{{ substr($po->sale_present_file, 14) }}</a>
                            @endif
                        </div>


                        <div class="mx-auto w-50 text-center mt-5">
                            <button type="submit" class="btn btn-primary btn-lg" name="button_add_submit">
                                تعديل
