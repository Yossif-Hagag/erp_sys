...
<head>
    ...
    <title>تعديل عملية مشتريات || العربي جروب</title>
    ...
    <div class="home overflow-hidden">
        ...
        <div class="content-body row m-0 overflow-y-scroll">
            ...
            <div class="content col-10 py-4 overflow-hidden">
                ...
                <form action="{{ route('update_operation_purchas', $Operationgpurchas->id) }}" method="POST"
                    ...
                    {{-- <select class="w-100 rounded-2 p-2 mb-3" id="transaction" name="operation_type">
                        ...
                        <option value="بيع"
                            {{ old('operation_type', $Operationgpurchas->operation_type) == 'بيع' ? 'selected' : '' }}>
                            بيع</option>
                        <option value="شراء"
                            {{ old('operation_type', $Operationgpurchas->operation_type) == 'شراء' ? 'selected' : '' }}>
                            شراء</option>
                    </select> --}}
                    ...
                    <div class="mb-4 text-end">
                        <label for="" class="form-label"><span class="text-danger">*</span>عرض السعر
                            المحفوظ</label>
                        @if (isset($Operationgpurchas->operation_document))
                            <a href="{{ asset('storage/' . $Operationgpurchas->operation_document) }}" target="_blank"
                                class="form-control text-bg-dark text-decoration-none">{{ substr($Operationgpurchas->operation_document, 11) }}</a>
                        @endif
                    </div>
                    <div class="mb-3 text-end">
                        ...
                        <input type="text" class="form-control" id="supplier-name" name = "client_name"
                            value="{{ old('client_name', $Operationgpurchas->client_name) }}" />
                        @error('client_name')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3 text-end">
                        ...
                        <input type="text" class="form-control" id="supplier-phone" name= "address"
                            value="{{ old('address', $Operationgpurchas->address) }}" />
                        @error('address')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3 text-end">
                        ...
                        <input type="number" class="form-control" id="transportation" name = "cost"
                            value="{{ old('cost', $Operationgpurchas->cost) }}" />
                        @error('cost')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
